...
def keep_img_size_open(path,size=(224,224)):
    img=Image.open(path)
    temp=max(img.size)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        img_path = os.path.join(self.path, segment_name.replace('png', 'jpg'))
        img = keep_img_size_open(img_path)
...
def get_train_images_auto(paths, height=224, width=224, flag=False):
    ...
    images = np.stack(images, axis=0)  #按照维度进行拼接列表
    print('uuuuuu___',images.shape)
    images = torch.from_numpy(images).float()    #torch.from_numpy(ndarray)的作用就是将生成的数组（数组是一般理解，标准应该称为array）转换为 张量Tensor，
    print('gggggggg',images.shape)
    images = (images-127.5)/127.5
    print('ttt___',images.shape)
    return images
